...
export function SlateTeal() {
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "'Inter', system-ui, sans-serif", background: "#F1F5F9", overflow: "hidden" }}>

      {/* Sidebar */}
      <aside style={{ width: 240, flexShrink: 0, background: "#1E293B", display: "flex", flexDirection: "column", padding: "24px 12px 20px" }}>
          ...
          {NAV.map(({ id, label, Icon }) => {
            ...
            return (
              <button key={id} style={{
                ...
                <Icon style={{ width: 15, height: 15, flexShrink: 0, color: active ? "#99F6E4" : "#64748B" }} />
                {label}
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", gap: 10, padding: "14px 8px 0", marginTop: 12 }}>
          <div style={{ width: 30, height: 30, borderRadius: "50%", background: "#0F172A", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#2DD4BF", flexShrink: 0 }}>A</div>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #E2E8F0", padding: "14px 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button style={{ padding: "7px 16px", borderRadius: 8, border: "1px solid #E2E8F0", background: "#fff", fontSize: 13, fontWeight: 500, color: "#1E293B", cursor: "pointer" }}>Export CSV</button>
            <button style={{ padding: "7px 16px", borderRadius: 8, border: "none", background: "#0D9488", fontSize: 13, fontWeight: 600, color: "#fff", cursor: "pointer" }}>+ Add Member</button>
          </div>
        </div>

        {/* Stats row */}
        <div style={{ background: "#fff", borderBottom: "1px solid #E2E8F0", padding: "12px 28px", display: "flex", gap: 24 }}>
          {[{ icon: BarChart2, label: "People Reached", value: "2,431", color: "#0D9488" }, { icon: Star, label: "Highlights", value: "14", color: "#D97706" }, { icon: MessageCircle, label: "Comments", value: "89", color: "#7C3AED" }].map(({ icon: Icon, label, value, color }) => (
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 28px", display: "flex", flexDirection: "column", gap: 14 }}>
          {POSTS.map((p, i) => (
            <div key={i} style={{ background: "#fff", borderRadius: 14, border: "1px solid #E2E8F0", borderLeft: p.highlight ? "3px solid #0D9488" : "1px solid #E2E8F0", padding: "18px 20px", boxShadow: "0 1px 2px rgba(30,41,59,0.04)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                <div style={{ width: 38, height: 38, borderRadius: "50%", background: "#F0FDFA", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14, color: "#0D9488" }}>{p.author[0]}</div>
                <div>
                ...
                {p.highlight && <span style={{ marginLeft: "auto", fontSize: 11, fontWeight: 600, background: "#F0FDFA", color: "#0D9488", borderRadius: 999, padding: "2px 8px" }}>★ Highlight</span>}
              </div>
