    ...
    (async () => {
      try {
        const res = await fetch("/api/users/login", {
          method: "POST",
          headers: { "Content-Type": "application/json", "X-Org-Subdomain": "calvary" },
          body: JSON.stringify({ email: "admin@calvary.org", password: "password123" }),
          credentials: "include",
        });
        if (cancelled) return;
        if (res.ok) {
          window.location.href = "/calvary/feed";
        } else {
    ...
    <div style={{
      ...
      {errMsg ? (
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ fontSize: 13, color: "#94A3B8", margin: 0 }}>Signing you in as a demo admin</p>
        </div>
      )}
